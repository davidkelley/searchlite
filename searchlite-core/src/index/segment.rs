...
fn collect_vector_value(
  ...
  // vector is persisted, then `l2_distance(v, origin)` accumulates `v[i]^2`
  // into `+inf` (post-BUG-388 it now saturates to `f32::MAX`, but the
  // similarity it produces is still meaningless against distant queries).
  // Guarding both metrics uniformly gives a diagnosable failure at write time
  // instead of persisting a vector that cannot be meaningfully compared.
